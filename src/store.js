...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    autoIncrementScoreNumber(state) {
      setInterval(function() {
        state.globalTimer =
          ...
          (state.atoutStore.cursorNumber / 10) * state.itemStore.blueNumber;
    ...
    bluecursorInteract(state) {
      let bluePrice = state.itemStore.bluePrice;
      state.scoreNumber = state.scoreNumber - bluePrice;
      state.itemStore.blueNumber++;
      VueCookie.set("bluecursor", state.itemStore.blueNumber);
    }
  },
  modules: {
    atoutStore,
